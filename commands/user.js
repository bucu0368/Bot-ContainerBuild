...
async function handleAvatar(interaction) {
    ...
    try {
        ...
        const embedShape = {
            ...
            image: { url: avatar1024 },
            timestamp: new Date(),
            footer: { text: `User ID: ${targetUser.id}` }
        };

        await replyV2(interaction, { embed: embedShape });
...
async function handleBanner(interaction) {
    ...
    try {
        ...
        if (fullUser.hexAccentColor) {
            fields.push({ name: 'Accent Color', value: fullUser.hexAccentColor, inline: true });
        }
