            ...
            try {
                ...
                try {
                    ...
                    if (newSearchResults.results && newSearchResults.results.length > 0) {
                        // Reload with new search (implementation would restart the whole process)
                        // For brevity, showing success message
                        const successContainer = new ContainerBuilder()
                            .setAccentColor(getEmbedColor(interaction.client))
                            .addTextDisplayComponents(
                                textDisplay => textDisplay
                                    .setContent(`✅ **New search completed!**\n\nFound results for **"${newQuery}"**. Please use the command again to view the new results.`)
                            );
